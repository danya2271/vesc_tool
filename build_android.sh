...
if [ "$SETUP_DEPS" = true ]; then
    ...
    # Ensure llvm-strip symlinks for NDK r23
    for arch in aarch64-linux-android arm-linux-androideabi x86_64-linux-android i686-linux-android; do
        strip_tool="$NDK_TARGET_DIR/toolchains/llvm/prebuilt/linux-x86_64/bin/${arch}-strip"
        llvm_strip="$NDK_TARGET_DIR/toolchains/llvm/prebuilt/linux-x86_64/bin/llvm-strip"
        if [ ! -f "$strip_tool" ] && [ -f "$llvm_strip" ]; then
            ln -sf llvm-strip "$strip_tool" 2>/dev/null || true
        fi
    done

    # Check and download build-tools 30.0.3 if no compatible build tools (<= 30.x with dx) exist
    BT_TARGET_DIR="$SDK_DIR/build-tools/30.0.3"
    HAS_COMPAT_BT=false
    if [ -d "$SDK_DIR/build-tools" ]; then
        for bt in "$SDK_DIR/build-tools/"*; do
            if [ -f "$bt/dx" ] || [ -f "$bt/lib/dx.jar" ]; then
                HAS_COMPAT_BT=true
                break
            fi
        done
    fi
    if [ "$HAS_COMPAT_BT" = false ]; then
        mkdir -p "$SDK_DIR/build-tools"
        TMP_BT_ZIP="/tmp/build-tools_r30.0.3-linux.zip"
        log_info "Downloading Android Build-Tools 30.0.3 (required for Qt 5.15 / dx)..."
        curl -L -o "$TMP_BT_ZIP" "https://dl.google.com/android/repository/build-tools_r30.0.3-linux.zip"
        log_info "Extracting Build-Tools..."
        TMP_BT_DIR="/tmp/bt30_ext_$$"
        mkdir -p "$TMP_BT_DIR"
        unzip -q "$TMP_BT_ZIP" -d "$TMP_BT_DIR"
        mv "$TMP_BT_DIR/android-11" "$BT_TARGET_DIR"
        rm -rf "$TMP_BT_ZIP" "$TMP_BT_DIR"
        log_success "Build-Tools 30.0.3 installed to $BT_TARGET_DIR"
    else
        log_info "Compatible Android build-tools already present."
    fi
...
# 5. Detect compatible Android Build Tools (Qt 5.15 / AGP 3.6 requires build tools <= 30.x with dx)
DETECTED_BUILD_TOOLS=""
if [ -d "$ANDROID_HOME/build-tools" ]; then
    for bt in "30.0.3" "30.0.2" "30.0.1" "30.0.0" "29.0.3" "29.0.2" "28.0.3"; do
        if [ -d "$ANDROID_HOME/build-tools/$bt" ]; then
            DETECTED_BUILD_TOOLS="$bt"
            break
        fi
    done
    if [ -z "$DETECTED_BUILD_TOOLS" ]; then
        for bt_dir in "$ANDROID_HOME/build-tools/"*; do
            if [ -f "$bt_dir/dx" ] || [ -f "$bt_dir/lib/dx.jar" ]; then
                DETECTED_BUILD_TOOLS="$(basename "$bt_dir")"
                break
            fi
        done
    fi
...
if [ -n "$DETECTED_BUILD_TOOLS" ]; then
    export ANDROID_BUILD_TOOLS_REVISION="$DETECTED_BUILD_TOOLS"
    log_info "Using Android Build Tools: $DETECTED_BUILD_TOOLS"
else
    log_warn "No Android build-tools with dx found; Gradle packaging may fail. Run --setup-deps to install build-tools 30.0.3."
fi

# Ensure llvm-strip symlinks exist in NDK for AGP compatibility
if [ -d "$ANDROID_NDK_ROOT/toolchains/llvm/prebuilt/linux-x86_64/bin" ]; then
    for arch in aarch64-linux-android arm-linux-androideabi x86_64-linux-android i686-linux-android; do
        strip_tool="$ANDROID_NDK_ROOT/toolchains/llvm/prebuilt/linux-x86_64/bin/${arch}-strip"
...
# 6. Detect Android Platform (prefer tested platforms 34 down to 28)
if [ -z "$PLATFORM_API" ]; then
    if [ -d "$ANDROID_HOME/platforms" ]; then
        for p in 34 33 32 31 30 29 28; do
            if [ -d "$ANDROID_HOME/platforms/android-$p" ]; then
                PLATFORM_API="android-$p"
                break
            fi
        done
        if [ -z "$PLATFORM_API" ]; then
...
# 7. Generate dynamic deployment settings JSON
...
cat > "$SETTINGS_FILE" <<EOF
{
   ...
   "qt": "$QT_ANDROID",
   "sdk": "$ANDROID_HOME",
   "sdkBuildToolsRevision": "${DETECTED_BUILD_TOOLS:-}",
...
build_apk() {
    ...
    if [ -n "$DETECTED_BUILD_TOOLS" ] && [ -f "$SETTINGS_FILE" ]; then
        sed -i 's/"sdkBuildToolsRevision": ".*"/"sdkBuildToolsRevision": "'"$DETECTED_BUILD_TOOLS"'"/' "$SETTINGS_FILE"
    fi
